...
function loadsongs(){
    ...
    ul.innerHTML="";
    bhajansongs.forEach(function(obj){
...
function playsong(){
    ...
    var flag = false;
    for(var i = 0; i < bhajansongs.length; i++){
        if(btn_id == bhajansongs[i].song_id){
            flag = true;
            audio_1.src = bhajansongs[i].song_url;
            helperFrwd =  bhajansongs[i].song_id;
            helperbcwrd = bhajansongs[i].song_id;
            helperF();
            helperB();
            break;
        }
...
function add_to_playList(){
    // console.log("function: called");
     ...
     for(var i = 0; i < bhajansongs.length; i++){
         if(song_id == bhajansongs[i].song_id){
             song_obj = bhajansongs[i];
             break;
         }
     }
     isPresent='false';
     if(song_obj.present_status == 'false'){
        for(var i = 0; i < playlist_song.my_play_list.length; i++){
            if(this.title == playlist_song.my_play_list[i].id && playlist_song.my_play_list[i].place == 'b'){
                isPresent='true';
                break;
            }
        }
        if(isPresent =='false'){
           playlist_song.addsong(song_obj.song_id , song_obj.song_name , song_obj.song_url , song_obj.song_img, song_obj.place );
           showPlaylist();
           song_obj.present_status = 'true';
        }else{
            alert("oops , you cant't add it again !! this song is already in your playlist ")
            return;
        }
     }
     else{
...
function showPlaylist(){
    ...
    playlist_song.my_play_list.forEach(function(obj){
        ...
        // this is for deteting item from my playlist
        icon_btn.setAttribute('title',obj.name)
        // --------------------------------------
...
function dlt(){
    ...
    // console.log("dlt-function-called");
     ...
     for(var i = 0; i < bhajansongs.length; i++){
        if(name == bhajansongs[i].song_name){
            songObj = bhajansongs[i];
            isFind=='true'
            break;
        }
      }
        ...
        if( isFind == 'false'){
            for(var i = 0; i < Hindisongs.length; i++){
                if(name == Hindisongs[i].song_name){
                    songObj = Hindisongs[i];
                    isFind=='true'
                    break;
                }
              }
     ...
     if(con){
        ...
        alert("You delete this song : "+songObj.song_name) 
    }else{
        //  alert("Okay")        
     }
...
function helperF(){
    ...
    if(frwdidx+1<bhajansongs.length){
        frwdidx=frwdidx+1;
        frwdbtn.disabled=false;
        // console.log(j);
    }else{
...
function forward(){
  ...
  ctrlflag=true;
   for(var i = 0; i < bhajansongs.length; i++){
       if(frwdidx+1 == bhajansongs[i].song_id){
        audio_1.src = bhajansongs[i].song_url;
...
function backward(){
    ...
    ctrlflag=true;
     for(var i =0; i < bhajansongs.length; i++){
         if(bcwrdidx+1 == bhajansongs[i].song_id){
            audio_1.src = bhajansongs[i].song_url;
            audio_1.play();
            bcwrdselfhelp();
            helperFrwd=bhajansongs[i].song_id
...
function searchOperation(){
    ...
    bhajansongs = bhajansongs.filter(function(obj){
        return obj.song_name.toLowerCase().includes(toSearch.toLowerCase());
    });
    // console.log(Hindisongs)
    loadsongs();
}
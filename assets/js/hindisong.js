...
function loadsongs(){
    ...
    ul.innerHTML="";
    Hindisongs.forEach(function(obj){
...
function playsong(){
    ...
    for(var i = 0; i < Hindisongs.length; i++){
        if(btn_id == Hindisongs[i].song_id){
            flag = true;
            audio_1.src = Hindisongs[i].song_url;
            helperFrwd =  Hindisongs[i].song_id;
            helperbcwrd = Hindisongs[i].song_id;
            helperF();
            helperB();
            break;
        }
...
function add_to_playList(){
    // console.log("function: called");
     ...
     for(var i = 0; i < Hindisongs.length; i++){
         if(song_id == Hindisongs[i].song_id){
             song_obj = Hindisongs[i];
             break;
         }
     }
     isPresent='false';
     if(song_obj.present_status == 'false'){
        for(var i = 0; i < playlist_song.my_play_list.length; i++){
            if(this.title == playlist_song.my_play_list[i].id && playlist_song.my_play_list[i].place == 'OH'){
                isPresent='true';
                break;
            }
        }
...
function helperF(){
    ...
    if(frwdidx+1<Hindisongs.length){
        frwdidx=frwdidx+1;
        frwdbtn.disabled=false;
        // console.log(j);
    }else{
...
function forward(){
  ...
  ctrlflag=true;
   for(var i = 0; i < Hindisongs.length; i++){
       if(frwdidx+1==Hindisongs[i].song_id){
        audio_1.src = Hindisongs[i].song_url;
...
function backward(){
    ...
    ctrlflag=true;
     for(var i =0; i < Hindisongs.length; i++){
         if(bcwrdidx+1 == Hindisongs[i].song_id){
            audio_1.src = Hindisongs[i].song_url;
            audio_1.play();
            bcwrdselfhelp();
            helperFrwd=Hindisongs[i].song_id
...
function searchOperation(){
    ...
    Hindisongs = Hindisongs.filter(function(obj){
